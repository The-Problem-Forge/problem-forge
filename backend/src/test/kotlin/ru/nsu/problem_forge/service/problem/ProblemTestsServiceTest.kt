...
class ProblemTestsServiceTest {
    ...
    @Test
    fun `getTestsPreview should start generation when no cache exists`() {
        // Given
    ...
    @Test
    fun `getTestsPreview should detect changes and regenerate`() {
        // Given
        val problem = createProblemWithMainSolution()
        ...
        // Mock file repository for calculateProblemChecksum
        every { fileRepository.findById(1L) } returns Optional.of(solutionFile) // solution file ID

        every { problemRepository.findById(problemId) } returns Optional.of(problem)
        every { problemUserRepository.findByProblemIdAndUserId(problemId, userId) } returns user

        // Set up cache with OLD checksum
        problemTestsService.previewGenerationCache[problemId] = oldPreview
        problemTestsService.problemChecksumCache[problemId] = "old_checksum" // Different from current
